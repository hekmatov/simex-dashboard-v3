...
| 021 | Free-text authoring | How should a separate Add static content workflow balance Free-text QMD source, validation, and production-equivalent preview without entering the chart wizard? | Proposed for master approval | A — Split Source + Production Preview (recommended) | static content, Free text, QMD, preview, Build |
| 022 | Image authoring | How should Image creation and editing separate durable assets and saved transforms from transient viewer interactions? | Proposed for master approval | A — Canvas + Transform Inspector (recommended) | static content, Image, crop, rotation, assets, accessibility |
| 023 | Static panels in Build and View | How should saved Free-text and Image panels preserve canonical composition while exposing authoring chrome only in Build? | Proposed for master approval | A — Content-led Canonical Panels (recommended) | static content, Build, View, fullscreen, responsive |
| 024 | Image in Audience | How should Image panels join passive 16:9 Audience output while Free text remains excluded from Present? | Proposed for master approval | A — Quiet Canonical Composition (recommended) | static content, Image, Present, Audience, 16:9 |

## Sequence

Sketches 001–020 are complete and approved. Sketches 021–024 are the isolated Step 7S proposal submitted to the V3 Design master task. Their recommended winners and rejected alternatives remain comparison evidence until master acceptance. Remaining low-risk integration choices use the recommended approved pattern and are recorded in [`INTEGRATION-DEFAULTS.md`](INTEGRATION-DEFAULTS.md), without allocating another sketch number. A new sketch is opened only for a material change to product scope, ownership, persistence, a major workflow, or primary information architecture.
