...
export default function ContentDetail({ item, contentDraftCoordinator, datasetProfile, geoData, onRename, renameBusy, renameError, ...managerProps }) {
  if (!item) return <p className="source-content-empty">Select an item to inspect its details.</p>;
  if (item.kind === "media") return <MediaDetail item={item} contentDraftCoordinator={contentDraftCoordinator} onRename={onRename} renameBusy={renameBusy} renameError={renameError} {...managerProps} />;
  return <DataSourceDetail item={item} contentDraftCoordinator={contentDraftCoordinator} datasetProfile={datasetProfile} geoData={geoData} onRename={onRename} renameBusy={renameBusy} renameError={renameError} {...managerProps} />;
}

export function buildContentRenameDraft({ dashboard, item, displayName, defaultDescription = "" }) {
  const draftId = `manager-rename-${item.kind}-${item.id}`;
