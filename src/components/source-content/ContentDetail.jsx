...
export default function ContentDetail({ item, datasetProfile, onRename, ...managerProps }) {
  if (!item) return <p className="source-content-empty">Select an item to inspect its details.</p>;
  if (item.kind === "media") return <MediaDetail item={item} onRename={onRename} {...managerProps} />;
  return <DataSourceDetail item={item} datasetProfile={datasetProfile} onRename={onRename} />;
}
