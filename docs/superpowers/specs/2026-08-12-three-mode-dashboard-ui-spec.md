...
| MODE-02 | Build | Exercise authoring across scenario, page, section, panel, constrained layout, and time-group scopes; selected-object drafts; truthful validation and persistence; the canonical dashboard in its real geometry. | A compressed preview standing in for the dashboard, free-form panel geometry, silent save/discard, or nonfunctional enabled actions. | T-MODE-02 |
| MODE-03 | Present/controller | Sole authority for Audience opening/reopening/ending, scene composition, order, layout, title, deliberate blank, time group, manual cues, Play/Pause, cadence override, Blackout/Restore, and connection recovery. | Dashboard-authoring mutation or reliance on Audience controls. | T-MODE-03 |
...
3. Hold constant the V3 configuration and dataset snapshot; scenario; active page; section expansion; panel order and presets; filters; synchronized-time group and position; chart state; page scroll anchor and scrollY; and any content that affects intrinsic size.
...
| PKG-08 | Delete Scenario | Explicitly excluded | The authoritative product contains one active dashboard scenario, not a multi-scenario collection. Page/section/panel/time-group removal remains in its owned scope. | Action is absent; no disabled placeholder implies multi-scenario deletion. | T-PKG-NO-DELETE |
...
| DISC-02 | Contextual | Common operations for the selected scope: primary metadata; add/move/reorder; panel data/content; supported layout presets; time-group membership and default cadence. | After selection, the relevant common actions are reachable without traversing unrelated advanced controls. | T-DISC-CONTEXT |
...
| Clause | Binding requirement | Observable pass condition | Test |
|---|---|---|---|
| PRES-01 | Controller exclusively owns scene selection, one-to-four chart composition, deterministic order/layout, title state, deliberate blank, time group, playback, cadence, Blackout/Restore, opening/reopening, and End Presentation. | Audience has zero interactive moderator actions and cannot mutate controller state. | T-PRES-AUTHORITY |
...
| PRES-11 | Previous, Next, direct seek, or time-group change during autoplay applies the requested cue and then leaves playback paused. | The resulting position/group is stable until explicit Play. | T-PRES-MANUAL-PAUSE |
...
| LIFE-12 | Reopen/reload sends one fresh complete snapshot and preserves chart set, order, layout, title, deliberate blank, time group/position, and blackout state. | Restored state ledger equals the pre-disconnect ledger; playback remains paused. | T-LIFE-REOPEN-PERSIST |
| LIFE-13 | Blackout/Restore is reversible and never changes composition, deliberate-blank choice, filters, time group, or saved dashboard state. | Before/after ledgers match except blackout and paused-playback state. | T-LIFE-BLACKOUT-REV |
...
| STATE-01C | Zero time groups | Use COPY-25 in Build; View/Present label synchronized playback unavailable because no time group exists. | Read-only structure entry that implies an editable group exists. | T-STATE-NO-TIME-GROUPS |
...
| COPY-25 | Zero time groups; Build | No time groups have been created. | Create Time Group | T-COPY-NO-TIME-GROUPS |
...
| FIX-CARDINALITY-ZERO — simex-ui-v3/cardinality-zero/r1 | Deterministic zero-shape script: start with one valid scenario and zero pages (COPY-22); Create Page produces one empty page with zero sections (COPY-23); Create Section produces one empty section with zero panels (COPY-24). Throughout the script there are zero time groups/members (COPY-25) and zero presentation catalogue items (COPY-26). |
| FIX-CARDINALITY-ONE — simex-ui-v3/cardinality-one/r1 | One scenario, one page, one section, one panel, one time group containing that one panel, one catalogue item, and exactly one supported layout preset for the panel. All names and values are fixed by this signature. |
| FIX-CARDINALITY-MANY — simex-ui-v3/cardinality-many-long/r1 | One scenario, three pages, eight sections, 40 panels/catalogue items, three time groups, six members in the largest group, and three supported layout presets for the selected panel. The final object in each collection uses FIX-LONG text. |
...
| FIX-LONG — simex-ui-v3/long-content/r1 | Fixed strings: 96-character scenario name, 72-character page label, 120-character section title, 128-character panel title, 80-character time-group name, 64-character legend category, and 240-character validation/error explanation. |
...
| TASK-B07 | Edit scenario, page, section, panel, layout, and time-group scopes, including membership, primary clock, datum/matching, and authored default cadence. |
...
| Control map — Time-group selection presents an inspectable object but no editing operation. | High | SCOPE-06, DISC-02, DISC-03 | TASK-B07 |
...
| COVER-09 | Playback controls; interactive-control | No compatible time group; first, middle, final position; manual cue; playing; paused; authored cadence; overridden cadence; safety pause; manual-intervention pause; loading/recovery; reason-disabled endpoints. | Empty collection dismissed: group absence is represented as unavailable playback with reason. | PRES-07 through PRES-12; T-COVER-PLAYBACK |
