...
- Unit Orbit correction: Stage 3 now supplies board-and-chart anchors to the shared floating `UnitOrbit` instead of appending a detached settings section after both canvases. The production journey asserts that the selected chart retains at least 240 px of visible height, the complete orbit remains inside the viewport, and their rectangles do not overlap. In-app inspection confirmed the selected map and the orbit were visible together with a 12 px clearance.
