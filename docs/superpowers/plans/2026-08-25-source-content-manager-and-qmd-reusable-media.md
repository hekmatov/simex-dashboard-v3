...
| Path | Exact exports and responsibility |
|---|---|
| `src/static-content/qmd/portableQmdMedia.js` | `parsePortableMediaReference(destination)`; `validatePortableMediaAttributes(attributes)`; `serializePortableMediaReference({mediaId,alt,width,align,flow,frame,caption,decorative})`; `annotatePortableMediaTokens(ast:PortableQmdAst) -> AnnotatedPortableQmdAst`; `parsePortableQmdWithMedia(source) -> ParsedPortableQmdResult`; `extractPortableMediaNodes(ast:AnnotatedPortableQmdAst,{mediaItems})`. This owner alone recognizes/consumes the immediate attribute suffix; it imports `parsePortableQmd`, while `parsePortableQmd` never imports this module, preventing a cycle. |
| `src/static-content/qmd/compilePortableQmd.js` | Modify `compilePortableQmd(source,options={})` to call `parsePortableQmdWithMedia(source) -> ParsedPortableQmdResult`, preserve invalid-parse errors/warnings/stats, and pass only non-null `parsed.ast` to `renderPortableQmd`. Its existing compiled-result shape remains unchanged. |
| `src/static-content/qmd/renderPortableQmd.js` | Modify `renderPortableQmd(ast:AnnotatedPortableQmdAst,options) -> DocumentFragment` to emit safe hosts only for known local records. Host attributes: `data-qmd-media-host`, `data-qmd-media-key`, and validated token data only. Unknown/external nodes emit inert source text. |
...
| `tests/helpers/geoJsonBoundaryFixtures.js` | `SOURCE_GEOJSON_LIMIT_KEYS` is exactly `['encodedBytes','features','totalPositions','renderableFragments']`; `geoJsonAtBoundary(metric,boundary,limits=GEOJSON_LIMITS)` accepts only those keys and `boundary:'normalMax'|'warningMin'|'hardMin'`, rejecting every removed metric and `concurrentMaps`; `concurrentMapScenario({dashboardMaps,previewMaps,visibleDashboardIds},limits=GEOJSON_CONCURRENT_MAPS)` is the separate scheduling fixture and never calls `geoJsonAtBoundary`. |
...
/** @typedef {{featureCount:number,geometryTypeCounts:Record<string,number>,boundingBox:[number,number,number,number]|null,propertyKeys:string[],encodedBytes:number,totalPositions:number,renderableFragments:number,diagnostics?:{maxPositionsPerFeature?:number}}} GeoJsonSummary */
/** @typedef {{ok:boolean,errors:Array<{code:string,path:string,message:string}>}} GeoJsonSchemaResult */
/** @typedef {{status:'normal'|'warning'|'rejected',facts:{encodedBytes:number,features:number,totalPositions:number,renderableFragments:number},warnings:string[],violations:string[]}} GeoJsonAdmissionResult */
...
Ordinary schema/compatibility validation is typed separately from admission: FeatureCollection/Feature structure, supported Point/MultiPoint/LineString/MultiLineString/Polygon/MultiPolygon coordinates, finite coordinates, required arity/nesting, line/ring minima and ring closure, and current-runtime GeometryCollection rejection are schema hard blocks; selected join-field absence and zero usable join coverage are direct-map compatibility blocks. `maxPositionsPerFeature` may appear only in `summary.diagnostics` as a non-blocking concentration warning. Property-key count is handled by searchable virtualized/paginated UI; nested property data is iterated or rendered lazily; property-value bytes are bounded by total source bytes; structural-node count is informational only. None can independently reject. Avoid eager recursive property expansion because the historical 50,000-container probe produced a 2,180 ms long task.
...
```powershell
$matrixText = Get-Content -Raw 'docs/audits/2026-08-24-v3-static-content-panels/FIDELITY-MATRIX.md'
$section = $matrixText -split '(?m)(?=^## )' |
  Where-Object { $_ -match '^## Final Step 7S controlling 36-row disposition(?:\r?\n|$)' }
if ($section.Count -ne 1) { throw "Expected one controlling Step 7S section; found $($section.Count)." }
$rows = $section -split '\r?\n' |
  Where-Object { $_ -match '^\| (FT-(0[1-9]|1[0-2])|IM-(0[1-9]|1[0-6])|PS-0[1-8]) \|' }
$expectedIds = @(
  1..12 | ForEach-Object { 'FT-{0:D2}' -f $_ }
) + @(
  1..16 | ForEach-Object { 'IM-{0:D2}' -f $_ }
) + @(
  1..8 | ForEach-Object { 'PS-{0:D2}' -f $_ }
)
$ids = $rows | ForEach-Object { [regex]::Match($_, '^\| ([^| ]+) \|').Groups[1].Value }
if ($rows.Count -ne 36) { throw "Expected 36 Step 7S rows; found $($rows.Count)." }
if (($ids | Select-Object -Unique).Count -ne 36) { throw "Step 7S row IDs are not unique." }
if (Compare-Object $expectedIds $ids) { throw "Step 7S row IDs do not match the exact accepted set." }
...
- [x] **GREEN:** Implement migration first; make `finalizeStaticContentDraft` and `prepareStaticPanelTransaction` match the exact contracts above. Change `StaticContentEditor` to resolve the current MediaItem, initialize the full placement/media/assets/staged contract, and forward the complete finalized payload to its existing `onSave` boundary. `DashboardRenderer` creates `ContentRenderContext`; `DashboardModeWorkspace` and `DashboardCanvas` pass it; `ChartPanel` carries it in `renderContext`; `ChartView` passes it to Image. `resolveStaticImageSource` validates placement, looks up `mediaItems[source.mediaId]`, and uses media revision/current. Durable commit publishes staged bytes and the single V5 candidate atomically and never forces V4. Do not serialize old Image shape.
- [ ] Re-run the exact RED command. Expected: PASS. Fresh result: 259/260 passed; the sole PS-04 failure combines current V5 promotion with the deliberately unrebuilt pre-Task-1 `dist`, while this task explicitly prohibits the full build needed to refresh that generated client. Source/package compatibility checks and both targeted Chromium files passed; see implementation evidence and Task-1 report.
...
**Rows:** SCM-S15 engine only. **Consumes:** `GEOJSON-LIMITS-DECISION.md`. **Produces:** frozen four-key `GEOJSON_LIMITS`, separate `GeoJsonSchemaResult`/`GeoJsonAdmissionResult`, combined `GeoJsonValidationResult`, and one lean `GeoJsonSummary` shape.

**Files:** Create `src/lib/geoJsonValidation.js`, `src/content-library/geoJsonSourceEntry.js`, `tests/helpers/geoJsonBoundaryFixtures.js`, `tests/geoJsonValidation.test.js`, `tests/geoJsonSourceEntry.test.js`. Modify `src/lib/loadDashboard.js`, `src/data/sourceRequest.js`, `src/data/dashboardSourceProviders.js`, `tests/progressiveDashboardLoad.test.js`, `tests/chartSourceProfile.test.js`, `tests/dashboardSemanticBoundary.test.js`, `tests/datasetProfilesV3.test.js`, and `tests/dataServiceFoundation.test.js`.

**Execution record (2026-08-25):** Task 2 RED, minimal GREEN, exact targeted verification, and SCM-S15 engine evidence update completed. The engine is implemented; manager composition, transaction rollback, mounted real-use, and browser evidence remain assigned to later tasks and are not promoted here.
...
- [x] **RED:** Assert pure rename semantics and the exact `ContentDraftCoordinator` API. Mount App→Renderer→ModeWorkspace→BuildWorkspace with one coordinator. At this pre-manager layer, test direct `discardDraft` (the callback used by explicit Cancel), mode departure, unmount, validation failure, persistence failure, and a synthetic owner:`manager` Add. The Add persists an unused content record across reload; every discard/failure exit leaves no durable item. QMD/Image/chart drafts publish only with completed owners. `getActiveRetainers` reports exact sorted IDs and contextual Image replacement lifetime. Task 4 mounts and tests Close/Escape; Tasks 7, 10, and 13 test each concrete upload flow.
- [x] Run `node --test tests/mediaItems.test.js tests/sourceEntrySchema.test.js tests/contentDraftTransaction.test.js tests/dashboardAppV3.test.js tests/buildWorkspaceV3.test.js tests/buildDirtyState.test.js tests/staticContentDraft.test.js tests/wizardDraftV3.test.js tests/browserAuthoredAssetStore.test.js tests/authoredAssetCleanup.test.js`. Observed: 68 pass / 6 fail; five intended missing-contract failures plus one inherited managed-sandbox Vite access denial.
- [x] **GREEN:** Implement the exact coordinator contract in `contentDraftTransaction.js`; create one instance in `App`, pass it through the exact prop path, subscribe Build dirty state to coordinator retainers, and dispose on App unmount. Implement rename functions as frozen copies. Coordinator commit builds one candidate, commits bytes/data and dashboard atomically, compensates on failure, then clears only the completed draft/transaction.
- [x] Re-run the exact command. Observed outside the restrictive filesystem sandbox: 85 pass / 0 fail / 0 skip / 0 todo.
- [x] Update engine/lifetime evidence and records; commit `feat(content): add scoped content draft publication`.
...
- [ ] **RED:** Assert serializer requires mediaId+alt and decorative yields empty accessible alt. Assert `parsePortableQmdWithMedia(source)` returns `ParsedPortableQmdResult`, preserving existing invalid-parse `errors`, `warnings`, and `stats`, and that `annotatePortableMediaTokens(ast)` returns an `AnnotatedPortableQmdAst` with the existing root fields plus media annotations. An accepted image consumes its immediately following fully allowlisted `{...}` suffix exactly once, removes that suffix from following text, and preserves remaining text. `compilePortableQmd` consumes the wrapper and passes only `parsed.ast` to `renderPortableQmd`; renderer and dependency extraction consume the same annotated AST. Malformed/raw URL/data/blob/file/unknown/external-only source stays visibly inert/request-free. Assert the full ContentRenderContext reaches QMD through DashboardCanvas/ChartPanel/ChartView. Assert healthy host/lease, known unhealthy logical fallback/repair, and exactly-one release/no orphan portal on recompile/unmount.
...
- [ ] **GREEN:** `parsePortableQmdWithMedia` calls the existing parser, returns its invalid result unchanged when `ast` is null, and otherwise returns a `ParsedPortableQmdResult` whose `ast` is `annotatePortableMediaTokens(parsed.ast)` while preserving diagnostics/stats. Only this module parses the immediate suffix. `compilePortableQmd` consumes the wrapper and passes `parsed.ast` to `renderPortableQmd`; renderer and dependency extraction consume `AnnotatedPortableQmdAst`. Renderer emits inert text for unknown/external and safe hosts for known local records. ChartView passes `contentRenderContext` to FreeTextChartView; QmdMediaView receives mediaItems/assets/resolveAsset/requestRepair. Portal and lease ownership remain exactly as mapped.
...
**Rows:** SCM-S05/S06/S07 and SCM-C08 delete branch. **Consumes:** Task 5 `parsePortableQmdWithMedia(source) -> ParsedPortableQmdResult` and `extractPortableMediaNodes(ast:AnnotatedPortableQmdAst,{mediaItems})`. **Produces:** exact saved-use/retention graph and blocked/eligible delete transaction.
...
- [ ] **GREEN:** Build graph from dashboard plus `contentDraftCoordinator.getActiveRetainers()`; parse QMD with `parsePortableQmdWithMedia` once and, only when `parsed.ast` is non-null, pass that `AnnotatedPortableQmdAst` to `extractPortableMediaNodes`. `prepareContentDeletion` returns `{status:'blocked'|'ready',directUses,retainers}`; `commitContentDeletion` accepts only ready plans and checks expected revision before atomic mutation.
...
- [x] **RED model/UI:** Focused Task 13 RED was 3/5 passing: uploaded-dataset GeoJSON was excluded and manager Add had no atomic transaction owner. The exact eleven-file RED subsequently exposed only the absent detail/preview/shared-budget wiring.
- [x] **RED budget:** Mounted assertions cover the sole common-ancestor provider, exact dashboard/preview requests, deterministic acquire/release/activation, dashboard priority, normal/degraded/deferred bands, and the negative admission/deletion boundary.
- [x] Run the exact eleven-file command. Observed RED before production; 114/119 passed and five Task 13 assertions failed.
- [x] **GREEN:** `DashboardModeWorkspace` owns the sole provider; chart and preview consumers share it. Manager and six-stage chart uploads publish through one atomic `contentDraftTransaction` candidate and discard staged authority on every non-completion exit.
- [x] Re-run the exact command. Observed: 119/119 PASS, zero fail/skip/todo.
- [x] Run `pnpm.cmd test:e2e tests/e2e/source-content-geojson.spec.js --project=chromium --grep "Journey I — GeoJSON upload select preview dependency and blocked delete"`. Observed: 1/1 PASS (48.7 s test, 50.7 s total) with inspected 1440×900/1024×768 Add/cancel inventories, exact six stages, eligible representations, summary/preview/fallback, shared four-slot behavior, direct breadcrumb, and blocked Delete/no dialog.
- [x] Updated S15/C09/R09 and focused rows only after inspection; commit `feat(content): manage GeoJSON with shared map budget`.
...
- [ ] **RED hard block:** Derive hard-boundary candidates only from the four `SOURCE_GEOJSON_LIMIT_KEYS`; assert malformed/empty/unsupported/schema-invalid/four-gate admission failure, selected join-property removal, zero join coverage, or directly unusable map blocks atomically and offers import-as-new/remap. Assert high property-key count, deep nested properties, high container diagnostics, and high per-feature concentration below the four gates cannot independently hard-block. Original descriptor/payload/summary/map stays exact.
...
| SCM-S03 — stable media identity/revision; explicit logical reuse; default only pre-fills new placements | `src/content-library/mediaItems.js`; `src/static-content/assets/browserAuthoredAssetStore.js`; `src/components/static-content/StaticContentEditor.jsx`; `src/content-library/contentDraftTransaction.js` | IDs, physical byte count, existing-edit V5 payload, revision, old/new alt | A default/dedupe branch | Build 1440×900; duplicate/default/existing edit | logical/physical counts, complete editor payload, and placement alts | Task 1 identity + Task 3 rename/publication engine implemented; manager/default/dedupe fidelity pending; complete row unpromoted | SCM-D03,D09 | 1,3,7,17 |
| SCM-S04 — trusted provenance visibility; generated hidden | `src/content-library/sourceEntrySchema.js` | adversarial filenames and explicit ownership | manager/picker origin/filter cases; D/I | Build 1440×900/1024×768 | exact visible/hidden IDs and labels | Task 3 classification engine targeted passing; manager/picker composition and fidelity not verified | SCM-D06 | 3,4,10,13,17 |
...
| SCM-S13 — manager explicit Add; authoring commit with panel/chart; contextual prior Image only | `src/content-library/contentDraftTransaction.js`; `src/static-content/assets/reconcileAuthoredAssets.js`; `src/components/build/buildDirtyState.js` | every commit/cancel/reset/restore boundary | A draft/Restore branch | Build 1440×900/1024×768; reload | durable/staged counts and exact lifetime | Task 3 coordinator/lifetime engine and mounted transport targeted passing; concrete flows/browser fidelity pending; complete row unpromoted | SCM-D05,D09 | 3,7,17 |
...
| SCM-S15 — canonical lean GeoJSON summary; schema and compatibility distinct from exactly four iterative admission gates | `src/lib/geoJsonValidation.js`; `src/content-library/geoJsonSourceEntry.js` | imported four-key boundaries; fragment counts; obsolete-metric negative; exact summary | I upload/preview/runtime | Build 1440×900/1024×768 | four facts/summary/schema outcome/rollback/lazy property UI/mounted consumer | Proposed / unimplemented / not verified | SCM-D06,D07 | 2,13,17 |
...
- [x] The complete Static Image sourceVersion-2/mediaId cutover, existing-edit `StaticContentEditor` wrapper, atomic payload, and Build/View/fullscreen/Present/Audience regression evidence are in Task 1.
