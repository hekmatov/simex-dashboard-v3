...
2. the current Chrono Group, synchronized-playback, temporal-provenance, and Present integration behavior compared with the future requirements in `2026-08-12-temporal-authoring-chrono-design.md`.
...
- **Source-confirmed:** temporal behavior was traced through the Build Chrono Group rail/inspector, chart-level time-sync settings, `timeSyncModel`, `temporalMatch`, `applyTimeContext`, playback controls/provider/reducer/view, provenance rendering, Present, Audience, and display-state code.
...
| `CREATE-INPUT-08` — atomic create | Require a single serialized validation/persistence transaction for chart, optional source, and Chrono Group proposal; retain the draft and last-good dashboard on failure. |
