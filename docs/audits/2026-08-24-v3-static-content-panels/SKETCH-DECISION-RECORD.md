...
## Slice 5 implementation disposition

- Sketch 023 Variant A is realized through the canonical saved-panel renderer. Build and View share content and saved geometry; Build alone exposes authoring. Fullscreen obtains the same model through the displayed-grid lookup and keeps Image active and Free-text internally scrollable.
- Hidden Image actions retain the accepted hover reveal plus focus-within and explicit touch reveal. Because the action layer is overlaid, reveal does not shift the image or panel geometry.
