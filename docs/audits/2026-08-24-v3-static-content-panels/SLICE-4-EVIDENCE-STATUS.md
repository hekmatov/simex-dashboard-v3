...
|---|---|---|---|
| Authored asset durability | Passing for content-addressed IndexedDB stage/atomic commit/read, dedup byte re-verification, typed storage failures, exact 24-hour staged-only orphan grace, reference protection, and per-window refcounted object URL leases | App durable commit/removal remain passing. Browser asset-origin `ChartView` resolution starts only inside its owning effect; Strict replay, source supersession, and unmount each release only their own attempt while synchronous SSR remains unchanged | Strict Chromium harness passes replay-discarded/current/superseded/unmounted/rejected and active-URL checks. Production at 1024×768 settles staged preview and reloads a durable Image whose active blob remains fetchable |
...
- Fix round 2/5 StrictMode journey: passed 1/1 at 1024×768 in 12.9 seconds. It inspected the staged preview, saved/reloaded IndexedDB Image, visible raster, `blob:` source, and byte readability of the active URL under the production root.
...
- Fix round 2/5 final directly affected StrictMode/resolver/Image/temporal run: 38/38 passed in 10.07 seconds. Supported impacted durability/App/package run: 48/48 passed in 13.80 seconds. The strict harness contributes 7/7 and uses distinct release-capable attempts plus active blob fetches.
...
- Final production build passed with 890 modules transformed in 9.30 seconds after the StrictMode ownership fix. Known advisories were unchanged: Three/Vanta classic-script notices, the existing mixed static/dynamic `ChartFootprintPicker` advisory, and the existing large-chunk advisory.
