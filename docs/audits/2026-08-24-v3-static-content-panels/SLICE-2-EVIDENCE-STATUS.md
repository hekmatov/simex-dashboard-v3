...
|---|---|---|---|
| FT-03–FT-06 portable QMD policy and safety | Passing: 25 policy/parser/renderer assertions and 2 real-browser sanitizer/sink assertions | Blocking errors, linked recovery guidance, stale preview, and exact-save revalidation implemented | Accepted/malicious corpus inspected in real browser; imported-source and exhaustive live limit/link activation states remain explicitly unclaimed where the matrix says so |
| FT-07–FT-10 authoring and canonical renderer | Passing: mounted editor/canonical-route semantics and overflow assertions | One `FreeTextChartView` is used by authoring preview, Build, View, and fullscreen; wide split, narrow tabs, focus continuity, and bounded overflow implemented | Production journey passed at 1440×900, 1024×768, and 768×900 |
...
|---|---:|---|---|
| `markdown-it` | 15.0.0 | MIT | Inert token AST; current line selected after the <=14.1.0/14.1.1 smartquotes denial-of-service range |
...
| `katex` | 0.18.4 | MIT | Bundled restricted math rendering; HTML-only output admitted, with MathML/SVG excluded |
...
- initial source revision 1, an iframe-edit blocking error with both forward rails disabled, unsaved edit isolation, Discard restoration, and saved edit revision 2;
- accepted semantic QMD, callout/table content, canonical preview, Build, View, and focused fullscreen;
...
No accepted UI, source, renderer, or security behavior changed. A phase-order ruling defers only the reload/import-dependent completion of FT-11 to Slice 4 and the protocol-injection half of FT-12 to Slice 6. Neither pending boundary is reported as Passing.
