...
| Canonical renderer and viewer | Passing for typed source resolution before tabular preparation, synchronous and asynchronous asset resolvers, alt/decorative DOM semantics, stable pending/error state, and separate transient zoom/pan | The same `ImageChartView` routes through authoring preview, Build, View, and fullscreen. Saved geometry is rotation → normalized SVG viewBox crop → outer contain/cover fit; action overlay reveals on hover, focus-within, and explicit touch without layout shift; Audience is passive by capability | DOM viewBox/rotation/fit attributes and the rendered matrix composition, Build/View/fullscreen, rest/hover/focus/touch reveal, transient zoom/reset, unchanged saved crop, and sibling survival under forced failure passed. Async pending→resolved, rejected, and superseded-request behavior passed in Chromium. Audience remains later-slice evidence |
...
Fix-round production reruns passed individually in 43.6 seconds at 1440×900, 27.3 seconds at 1024×768, and 25.8 seconds at 768×900. The sole retained skip is `IM-06 reload continuation restores the original asset and saved transform`, annotated `Blocked by Slice 4: authored IndexedDB durability and dashboard/bundle v4 reload are not part of Slice 3.` It was not selected by the three viewport-specific rerun commands and remains the only intentional `fixme`.

## Verification disposition

- Focused and directly impacted Image/static/legacy-pipeline checks: 160/160 passed in the final post-refactor run in 7.44 seconds, including 3 Chromium decoder/async-resolver checks.
- Production build: passed, 883 modules transformed, 10.87 seconds; only the existing non-module-script, mixed static/dynamic import, and chunk-size warnings remain.
