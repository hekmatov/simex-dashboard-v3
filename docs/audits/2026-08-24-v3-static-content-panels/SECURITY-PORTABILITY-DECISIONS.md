...
- SP-12 and SP-14 are passing for the implemented engine/UI boundary. Integer-permille crop, quarter-turn rotation, fit/reset, replacement undo, required nondecorative alt, and explicit decorative semantics share the typed source/draft pipeline and canonical renderer. Saved output composes rotation before normalized viewBox crop and outer contain/cover fit; the retained browser journey verifies the rendered DOM matrix composition rather than metadata-only clipping.
