...
- The chart wizard keeps the current upload's staged draft/candidate/retainer authoritative while an existing, manual, or uploaded replacement awaits confirmation. Cancel discards only pending authority; confirmation discards current authority before adoption; Close/Escape suspension preserves the complete active/pending authority and UI state; confirmed discard, unmount, reset, and failure clean the applicable staged authority; completion requires and commits one identity-matched active source/profile/SourceEntry draft with the finalized chart.
- The corrected named Journey D is **1/1 passing**, 41.4 s test time and 43.3 s total. Before the upload branch, the mounted six-stage wizard selects the manager-created existing CSV and observes its profiled `capacity` column while the exact durable CSV inventory remains unchanged; the chart upload then survives Close/Resume and completes through the same six-stage draft.
...
- A mounted existing managed CSV was selectable in the exact six-stage flow without changing inventory. A chart upload remained absent from durable inventory, survived Close/Resume with its selected profile intact, and then atomically published one source/profile/entry with one completed chart through the same exact six-stage draft.
...
- Fresh exact GREEN after the task-scoped correction is **120/120 passing**, zero failed/skipped/todo. It covers the four-gate authority, canonical lean summary, high-property/nested-property non-blocking behavior, atomic Add/cancel/failure inventories, builder-managed tracked/packaged/uploaded eligibility with dashboard-owned generated GeoJSON excluded, staged suspend retention plus explicit replacement/unmount disposal, exact six-stage chart authoring, direct dependencies/blocked deletion, common-ancestor budget allocation/activation/release/priority, visible degraded status, and live chart transport.
...
- The journey retained a staged GeoJSON option and selection through Escape/Resume, then explicitly selected an existing managed GeoJSON and proved that supersession discarded the staged option. It also proved dashboard-owned generated GeoJSON absent from the builder picker while tracked, packaged, and uploaded items remained eligible.
...
## Beta pre-release closure evidence — 2026-08-26

- The authorized regression closure reached focused deterministic GREEN at **148/148**, with the bounded passive CSV-detail assertion repaired from **4/5 RED to 5/5 GREEN** and final combined four-file verification at **153/153** outside the known Vite sandbox. Individual Journey D and Journey I are each **1/1 passing**, and the explicitly authorized legacy chart-authoring fixture is **10/10 passing** against the exact accepted six-stage workflow.
- The production refresh build and governing build gate are GREEN at 925 modules (9.76 s and 9.26 s). The exact Task 17 73-file selection ran once outside the sandbox at **673/674**; its sole failure was the subsequently corrected passive CSV-detail assertion. It was not rerun.
- PS-04 passed in that exact selection after the generated-client refresh, including copied Windows launch, PNG MIME, traversal rejection, offline Audience, and cleanup. This supersedes the historical PS-04 residual only.
- The governing full unit gate remains red at **1,516/1,558 passing, 42 failing**. The full Playwright restart stopped on a repeated common stale `Program label` fixture in `dashboard-edit-race.spec.js` after **10 passed / 2 failed / 174 not run**.
- No amendment row is promoted by this closure. Existing task-local evidence is retained, but integrated acceptance and master submission remain pending.

## Focused pre-release continuation evidence — 2026-08-27

- The two stale dashboard-edit race fixtures now exercise the live debounced Build Inspector Page-title owner and pass **2/2** while retaining the save-order and Reset race assertions.
- The original 42 unit failures were grouped by observed stack/owner evidence and corrected at their owning boundaries: V5 presentation/static fixtures, QMD Vite/React runtime setup, canonical plot identity, tree semantics, Studio-context expectation, terminology-policy scope, and JSX loaders. Every recoverable cluster passed its focused owner selection. Four exact titles were absent from the truncated reporter and remain explicitly unreconstructed; no broad unit rerun was performed merely to backfill output.
- Browser continuation used failed/unrun-only selections with development fail-fast. Cumulative result: **186/186 passing, 0 failing, 0 not run**. Journeys A–K each passed independently; the retained checks cover manager/canvas continuity, responsive detail composition, QMD media geometry and safety, transactional replace/delete/recovery, CSV temporal handling, GeoJSON map limits/replacement, V5 offline portability, and missing/corrupt repair.
