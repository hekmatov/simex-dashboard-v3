...
| Canonical rendered chart date evidence | `src/components/charts/ChartView.jsx::chartActiveDate, renderChartContent` | Project an already-owned finite temporal context to normalized `YYYY-MM-DD` on the canonical chart frame. Static types have no context and therefore no marker; this is evidence on the existing renderer, not a new time owner. |
...
| Portable package promotion | `scripts/promote-dashboard-bundle.mjs::preparePromotedDashboard, promoteDashboardBundle, assertWithinPublicDirectory`; `scripts/build-portable-data.mjs::buildPortableData`; `scripts/package-flashdrive.mjs::packageFlashDrive` plus generated `start-dashboard-server.ps1::Get-ContentType` and path-containment block; `README.md` package section and generated `START_HERE.md` | Materialize local payloads under safe generated paths, add WebP MIME, preserve the containment check, distinguish tracked packaging from browser storage, and document the exact app export → root `packaged-dashboard-bundle.json` → `promote:bundle` → `package:flashdrive` operator sequence. Explicit output ownership enables a copied-package launcher test without deleting an existing target; the normal CLI behavior remains compatible. |
...
| Present selection and publishing | `src/components/presentation/PresentWorkspace.jsx::configuredChartGroups, toggleChart, reconcilePresentDisplayState, presentationState`; `src/components/DashboardRenderer.jsx::presentationValidChartIds`; `src/components/presentation/usePresentationRuntime.js::usePresentationRuntime, publish, open`; `src/lib/displayController.js::reduceDisplayState` | Build a complete-source trusted presentable-item index, list Image but not Free text, synchronously normalize selected items plus layout, and project ordered protocol descriptors from internal IDs. |
| Protocol and reconnect | `src/lib/presentationProtocol.js::PRESENTATION_PROTOCOL_VERSION, makePresentationMessage, parsePresentationMessage, validatePresentationState, reconcilePresentationState`; `src/lib/presentationChannel.js::createPresentationControllerChannel, createPresentationAudienceChannel` | Validate protocol-v3 chart/Image descriptors, synchronously filter stale identity/revision and normalize layout at publish, and replay only the exact still-trusted snapshot. |
| Present playback ownership | `src/charting/time/playbackReducer.js::initialPlaybackState, reducePlaybackState`; `src/components/presentation/PresentWorkspace.jsx::PresentWorkspace` | Track owner tokens while preserving legacy open/close semantics. A Present StrictMode mount owns one stable token and releases only that token, so pre-existing or overlapping owners remain open. |
...
| PS-04 | `tests/staticContentPortablePackage.test.js`; `tests/imageChartPackageSource.test.js`; `tests/portableFlashdriveLaunch.test.js` | `tests/portableFlashdriveLaunch.test.js` actually copies/launches the Windows package, runs offline main + separate Audience, and verifies process/temp cleanup |
