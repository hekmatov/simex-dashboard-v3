...
| QMD parser/renderer | `src/static-content/qmd/portableQmdPolicy.js::PORTABLE_QMD_POLICY, validatePortableQmdAst`; `src/static-content/qmd/parsePortableQmd.js::parsePortableQmd`; `src/static-content/qmd/renderPortableQmd.js::renderPortableQmd`; `src/static-content/qmd/compilePortableQmd.js::compilePortableQmd`; `src/components/charts/FreeTextChartView.jsx::FreeTextChartView` | The single parser recognizes validated `simex-media:media-id` nodes. The renderer emits production-owned safe hosts; FreeTextChartView appends the cloned fragment, collects committed hosts, and mounts/unmounts `QmdMediaView` with React portals. Unknown/external forms remain inert text/request-free. |
| CSV temporal review | `src/charting/time/chronoGroupModel.js::validateChronoGroups`; `src/charting/time/dashboardTemporalConfig.js::normalizeDashboardTemporalConfig, validateCanonicalDashboardTemporalConfig`; `src/charting/time/sceneSchema.js::validateScene`; `src/charting/time/temporalNeedsAttention.js::deriveTemporalNeedsAttention, deriveTemporalContentItems`; `src/components/time/chronoContentState.js`; `src/components/build/BuildWorkspace.jsx::dispatchChronoGroup, dispatchScene`; `src/components/presentation/PresentWorkspace.jsx::PresentWorkspace`; dashboard V5 migration/bundle/package owners | Persist exact optional review metadata on Chrono Group, top-level Scene, and Scene Present composition; project it into Build/Present without adding protocol/action/message fields. Successful Chrono save clears that group's review IDs; successful valid Scene save clears Scene and Scene-present IDs. |
...
| `src/static-content/qmd/portableQmdMedia.js` | `parsePortableMediaReference`, `validatePortableMediaAttributes`, `serializePortableMediaReference`, `extractPortableMediaNodes`: exact local-media grammar/attributes and parser-backed dependency extraction; no second regex grammar. |
...
| `src/components/build/BuildMapBudgetContext.jsx` | `createBuildMapBudget`, `BuildMapBudgetProvider`, `useBuildMapBudgetSlot`: one Build-workspace slot registry shared by live dashboard maps and manager previews; visible dashboard maps are preferred, slots 1–2 are normal, 3–4 degraded, and later requests are deferred until activation. |
...
- GeoJSON limit calibration: master-accepted at `c28b59d`; the 32 MB warning/36 MB hard byte-property boundaries retain demonstrated margin below the 48 MB failure, exclude no legitimate fixture, and create no user-level tradeoff.
...
- Plan review history: first plan `ebdc52b` was rejected because it left Static Image consumers unwired, ordered QMD consumers before the grammar/runtime, conflated picker import lifetimes, omitted manager/GeoJSON/temporal owners, oversized tasks, and contained undefined/mechanically incorrect checks. The corrected plan resolves those defects.
