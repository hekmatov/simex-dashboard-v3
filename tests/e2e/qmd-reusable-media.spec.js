...
test("Journey C — QMD media controls responsive RTL geometry and request authority", async ({ page }) => {
  ...
  await inspector.getByRole("button", { name: "Change image" }).click();
  await page.getByRole("region", { name: "Media picker" }).getByLabel(/Alternate map/).evaluate((node) => node.click());
