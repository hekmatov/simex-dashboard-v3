...
test("editor debounces parsing, keeps the last valid preview stale on a complexity error, and recovers without losing source", async () => {
  ...
  const editor = await portableQmdRepairSource();
...
test("routed controls wait for analysis, accept arbitrary inert text, and still block a complexity breach", async () => {
  ...
  assert.equal(await wizard.getByLabel("Rendered preview").locator("script, iframe").count(), 0);
  assert.equal(await wizard.getByLabel("Rendered preview").getByText(/<script>window\.authoredCodeRan/).count(), 1);
...
test("change then rapid revert restores cached validation and matching source/preview revisions", async () => {
  ...
  assert.equal(await page.getByLabel("Rendered preview").getByText("Initial valid preview.").count(), 1);
});

test("responsive writer, preview, and Portable Markdown cards remain visible and keep editor focus", async () => {
  await page.evaluate(() => window.mountFreeTextEditor("## Situation\n\nResponsive content."));
  const writer = page.getByLabel("Text post editor");
  const preview = page.getByLabel("Rendered preview");
  const markdown = page.getByLabel("Portable Markdown");
  await preview.getByText("Responsive content.").waitFor();
  const editor = page.getByLabel("Portable QMD Composer editing area");
  assert.equal(await page.locator("#portable-qmd-composer-surface").count(), 1);
  assert.equal(await page.locator("#portable-qmd-composer-focus-target").count(), 1);
  await editor.focus();
  await page.setViewportSize({ width: 768, height: 900 });
  await page.waitForTimeout(50);
  assert.equal(await writer.isVisible(), true);
  assert.equal(await preview.isVisible(), true);
  assert.equal(await markdown.isVisible(), true);
  assert.equal(await page.getByRole("tab").count(), 0);
  assert.equal(await page.evaluate(() => document.activeElement?.getAttribute("aria-label")), "Portable QMD Composer editing area");
...
test("canonical renderer and editor reject one-token math expansion before mounting over-budget DOM", async () => {
  ...
  await page.getByLabel("Rendered preview").getByText("Last valid preview.").waitFor();
  await page.evaluate((qmd) => window.setFreeTextEditorSource(qmd), source);
  await page.waitForFunction(() => /blocking error/i.test(document.querySelector("output[data-validation-ok]")?.textContent ?? ""));
  assert.match(await page.locator("output[data-validation-ok]").textContent(), /blocking error/i);
  assert.match(await page.locator(".free-text-validation-errors").textContent(), /DOM nodes.*5000/i);
  assert.equal(await page.getByLabel("Rendered preview").getByText("Last valid preview.").count(), 1);
  assert.deepEqual(pageErrors, []);
});
