...
## T6-R01–R04 integrity correction

- Correction BASE: `109b3ca1f7f03099e4783e77b6aa6545d9792f81`. The master explicitly authorized `SourceContentWorkspace.jsx::visibleManagerItems/contentItem` as the sole existing carrier boundary; this resolves the earlier owner-boundary finding without rolling back the post-validation transient projection.
- **R01:** one `classifyManagedSource` authority now recognizes tracked `kind: "csv"/"geojson"` and uploaded dataset `type: "uploadedCsv"/"uploadedGeoJson"` forms. Both the dependency graph and DashboardRenderer projection use it. Valid SourceEntry direct-use and mounted carrier regressions cover the dataset forms.
- **R02:** source deletion plans freeze a stable current-authority identity over the durable SourceEntry, authoritative descriptor, and CSV profile where applicable. Commit re-reads that identity and rejects name, provenance, descriptor, or profile drift while allowing unrelated dashboard changes. No SourceEntry revision was invented; GeoJSON carries no profile authority.
- **R03:** mounted deletion uses `browserAuthoredAssetStore` exact snapshot/remove/restore primitives. Candidate dashboard publication precedes physical deletion; the candidate asset-reference graph retains shared bytes, and dashboard or byte-delete failure compensates both dashboard and bytes.
- **R04:** the retained eligible browser fixture is a schema-valid external HTTPS MediaItem with no authored bytes. A focused RED proved projected `uses: []` was incorrectly treated as unknown; GREEN makes the transient carrier expose known zero use. Tablet RED then proved the detail action was outside the viewport; the bounded detail pane now scrolls without changing the desktop composition.
- Final exact deterministic selection: **26 passed / 0 failed / 0 skipped / 0 todo**, 3.546 s.
- Final planned Chromium selection, run as `node_modules/.bin/playwright.CMD test tests/e2e/source-content-manager.spec.js --project=chromium --grep "desktop composition|tablet composition"` with no literal separator: **2 passed / 0 failed**, 40.4 s, at 1440×900 and 1024×768. Both viewports prove used managed-source Delete is disabled with inline guidance and no dialog; eligible Delete opens the correctly named modal, focuses Cancel initially, Cancel preserves exact localStorage equality, and focus returns to Delete.
- **SCM-S07:** Passing for the bounded deletion slice with current-authority drift checks, browser-byte compensation, and actual mounted blocked/eligible behavior. **SCM-C08:** delete branch Passing; the row remains Partial for Task 8 replacement/relink. No Task 7+ work or parent `progress.md` update is included.
