...
- Focused correction GREEN: `node --test tests/portableQmdMedia.test.js` — **8 passed / 0 failed**; `node --test tests/freeTextChartView.test.js` — **15 passed / 0 failed**. The latter includes inert-code-before-real, angle-destination, duplicate selected-instance, and mounted picker-focus facts.
- Fresh exact deterministic command: `node --test tests/qmdMediaInspector.test.js tests/portableQmdMedia.test.js tests/qmdMediaView.test.js tests/freeTextChartView.test.js tests/portableQmdDomSafety.test.js` — **38 passed / 0 failed / 0 skipped / 0 todo**, 12.939 s.
- Exact named command, without a literal separator: `pnpm.cmd test:e2e tests/e2e/qmd-reusable-media.spec.js --project=chromium --grep "Journey C — QMD media controls responsive RTL geometry and request authority"` — **1 passed**, 3.6 s test time and 5.4 s total.
...
- The single annotated QMD parse owns each real media node's exact original `sourceStart`/`sourceEnd`, including one validated immediate suffix, and propagates that identity through the render host into authoring selection. The editor no longer derives identity from a key regex or searches canonical text with `source.indexOf`; it slices only those parser-owned offsets. Inert code, angle-bracket destinations, duplicate literals, unedited raw spelling, and suffix consumption are covered explicitly.
