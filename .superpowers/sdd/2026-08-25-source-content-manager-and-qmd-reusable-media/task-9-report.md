...
- T9-R01/R02 correction RED was established before correction production: the parser-focused run had **5 passed / 3 failed** because annotated nodes/tokens had no exact source spans and angle-bracket spelling was canonicalized. Mounted tests also encoded the missing autofocus, Escape, Close, and selection-return contracts before the focus implementation.
- Focused correction GREEN initially reached `portableQmdMedia` **8/8** and `freeTextChartView` **15/15**, covering inert-code-before-real, angle-destination, duplicate selected-instance, and mounted picker-focus facts.
- Final T9-R01 canonical-parser RED: `portableQmdMedia` was **8/9** because a valid reference-style local image had no scanner-derived span; `freeTextChartView` was **15/16** because selecting that reference placement made the edit a no-op. Inline placements still passing demonstrated the precise scanner gap.
- Final focused GREEN: `node --test tests/portableQmdMedia.test.js` — **9 passed / 0 failed**; `node --test tests/freeTextChartView.test.js` — **16 passed / 0 failed**. Reference, inline, and duplicate placements each edit only the selected canonical token while unedited raw spelling and the reference definition remain byte-identical; unresolved and unsafe references create no portable media node.
- Fresh exact deterministic command: `node --test tests/qmdMediaInspector.test.js tests/portableQmdMedia.test.js tests/qmdMediaView.test.js tests/freeTextChartView.test.js tests/portableQmdDomSafety.test.js` — **40 passed / 0 failed / 0 skipped / 0 todo**, 13.885 s.
- Exact named command, without a literal separator: `pnpm.cmd test:e2e tests/e2e/qmd-reusable-media.spec.js --project=chromium --grep "Journey C — QMD media controls responsive RTL geometry and request authority"` — **1 passed**, 3.7 s test time and 5.5 s total.
...
- The parser owner wraps Markdown-it's actual canonical `image` inline rule and records `state.pos` before and after every successful image-token creation. Relative positions are projected only through the owning inline token's bounded parser source-map range; no second image regex or manual image grammar remains. Portable media annotation consumes those exact token offsets, extends through one validated immediate suffix, and propagates identity through the render host into authoring selection.
- The editor no longer derives identity from a key regex or searches canonical media text with `source.indexOf`; it slices only parser-owned offsets. Reference-style and inline destinations, inert code, angle/escape spelling, duplicate literals, unedited raw bytes, and exact suffix consumption are covered explicitly.
- Every inspector change serializes through `serializePortableMediaReference` and replaces exactly the selected node span. Decorative serialization empties alt while visible caption remains separate.
...
- Focus remained on the selected placement, progressive More control, and Open action at their inspected checkpoints. Change moved focus into the mounted picker on the selected response item; Escape and explicit Close each returned focus to the Change trigger; reopening restored picker focus; and choosing the alternate returned focus to Change before the placement was reselected. The placement changed from `response` to `alternate`, Open routed `alternate`, and the exact media-library snapshot—including revisions—remained unchanged.
...
- **SCM-C06:** Passing for exact progressive controls, parser-owned selected-placement identity, allowlisted serialization, mounted picker focus/return context, placement-only Change, and Open routing.
...
- **SCM-R03:** Passing for the named Journey C Build/View/fullscreen fidelity and keyboard-focus contract.
