...
| Task 2 self | QMD parser/sanitizer/editor/renderer/styles/dependencies plus one retained journey | Internally consistent; live route depends on Task 1 shell and transaction. |
...
Task 2: fix round 1/5 complete (6 addressed, 0 open — actual sanitized-fragment node budget and exact boundaries; structurally usable generated HTML-only math; immediate pending/revision gates and rapid-revert cache; closed Markdown/raw-HTML allow-list; occurrence-safe footnotes/source lines; singular static CSS ownership; FT-11 reload/import continuation explicitly pending Slice 4; FT-12 protocol-injection continuation explicitly pending Slice 6)
Task 2: dependency rationale corrected: GHSA-38c4-r59v-3vqw is linkify ReDoS and GHSA-6v5v-wf23-fmfq is smartquotes quadratic DoS; exact `markdown-it@15.0.0` plus disabled linkify/typographer remains the approved conclusion.
