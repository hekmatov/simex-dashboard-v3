...
- Ruling: an already visible Build reveal target is not smooth-scrolled — why: the redundant animation can outlive modal-close restoration and caused a deterministic two-pixel 768×900 drift — cost if wrong: an offscreen target still uses the existing smooth reveal, while a visible target completes immediately.
